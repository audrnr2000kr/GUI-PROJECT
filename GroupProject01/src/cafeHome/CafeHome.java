...
public class CafeHome extends JFrame { // 첫화면
	...
	public CafeHome() {
		...
		btnMem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) { // 회원버튼 클릭
				// 회원버튼 누르면 로그인 창으로 이동
				setVisible(false);
				new CafeLogin().setVisible(true);
			}
		});
		btnMem.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		btnNonMem.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) { // 비회원버튼 클릭
				setVisible(false);
				new CafeMenu2().setVisible(true);
			}
		});
		btnNonMem.setFont(new Font("맑은 고딕", Font.BOLD, 14));
		...
		btnUpdate.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) { // 회원정보수정 버튼 클릭
				// 회원정보수청 화면으로 이동
				setVisible(false);
		...
		btnWithdrawal.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) { // 화원탈퇴 버튼 클릭
				// 회원탈퇴 화면으로 이동
				setVisible(false);
